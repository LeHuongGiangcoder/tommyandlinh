...
const TravelInfo = ({ lang = 'en' }: { lang?: 'en' | 'vi' }) => {
  ...
  const t = {
    ...
    vi: {
      ...
      hanoiDesc: "Hà Nội là nơi tụi mình rất mong mọi người được trải nghiệm trọn vẹn từ những địa điểm nổi tiếng như Văn Miếu, Hồ Hoàn Kiếm, Lăng Bác đến khu phố cổ nhộn nhịp. Chỉ cần 2 ngày 1 đêm là đủ để cảm nhận nét đặc trưng của thành phố và thưởng thức rất nhiều món ngon. Tụi mình có thể tổ chức đi theo nhóm — nhớ RSVP nếu bạn quan tâm nha!",
      halong: "Vịnh Hạ Long — Du thuyền qua đêm",
      halongDesc: "Đây là một trong những điểm đến đẹp nhất Việt Nam và cũng là trải nghiệm tụi mình rất recommend. Hành trình 2 ngày 1 đêm sẽ đưa bạn qua hàng ngàn đảo đá vôi tuyệt đẹp, kèm theo các hoạt động như chèo kayak, bơi lội và thư giãn trên tàu. Sẽ là một trải nghiệm rất đáng nhớ!",
      ninhbinh: "Ninh Bình — Thiên nhiên & lịch sử",
      ninhbinhDesc: "Ninh Bình nổi tiếng với cảnh quan núi đá vôi, sông nước và ruộng lúa xanh mướt — thường được gọi là “Vịnh Hạ Long trên cạn”. Bạn có thể đi thuyền xuyên hang, tham quan chùa Bái Đính và các di tích lịch sử cổ.",
          ...
          <div className="travel-card relative group visa-reveal">
            ...
            <div className="bg-white/60 backdrop-blur-sm border-double border-4 border-olive/10 p-8 md:p-16 relative z-10 shadow-sm transition-all hover:bg-white/80">
              ...
              <div className="space-y-16 text-ink/80 font-light leading-relaxed">
                <div className="relative pl-6 md:pl-10 border-l-[0.5px] border-olive/30 max-w-4xl">
                  <div className="px-6 md:px-8 py-6 bg-olive/[0.05] border-l-4 border-olive/30 rounded-r-sm max-w-2xl shadow-sm">
                    <p className="text-[16px] md:text-[18px] italic text-olive/90 font-serif leading-relaxed">{t.visaExemption}</p>
                  </div>
                </div>

                {/* Health Declaration / PAI Portal Notice - NO BOX, FULL WIDTH */}
                <div className="max-w-4xl pt-4">
                  <div className="flex items-start gap-4 mb-6">
                    <div className="text-burgundy flex-shrink-0 mt-1">
                      <FileText className="w-5 h-5 md:w-6 md:h-6" />
                    </div>
                    <div>
                      <span className="text-[10px] tracking-[0.4em] uppercase text-olive font-bold block mb-2">
                        {t.healthDeclTitle}
                      </span>
                  ...
                  <p className="text-[16px] md:text-[17px] text-ink/90 font-light leading-relaxed mb-8">
                    {t.healthDeclDesc}
                  </p>

                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-8 border-t border-olive/10">
                    <div className="space-y-2">
                      <span className="text-[11px] font-bold tracking-[0.2em] uppercase text-olive block">
                        {lang === 'en' ? 'How to Apply:' : 'Cách Thực Hiện:'}
                      </span>
                      <p className="text-[14px] md:text-[15px] text-ink/80 font-light leading-relaxed">
                        {t.healthDeclHow}
                      </p>
                    </div>
